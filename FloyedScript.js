...
document.addEventListener('DOMContentLoaded', (event) => {
    const InList = ['addvertex', 'addedge', 'fromVertex', 'toVertex', 'weight', 'calculate','newgraph'];
    let currentIndex = 0;

    document.addEventListener('keydown', (event) => {
        if (event.key === 'ArrowRight') {
            // Move focus to the next element
            currentIndex = (currentIndex + 1) % InList.length;
            document.getElementById(InList[currentIndex]).focus();
        } else if (event.key === 'ArrowLeft') {
            // Move focus to the previous element
            currentIndex = (currentIndex - 1 + InList.length) % InList.length;
            document.getElementById(InList[currentIndex]).focus();
        }
